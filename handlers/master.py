...
async def finish_master_registration(message: types.Message, state: FSMContext):
    ...
    # 2. Генерируем персональную ссылку
    ...
    await message.answer(
        ...
        f"<i>Просто скопируйте ссылку и вставьте её в Instagram.</i>",
        parse_mode="HTML",
...
async def view_all_records(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        await message.answer("🔍 Функция выгрузки всех записей будет доступна в админ-панели.")
    else:
