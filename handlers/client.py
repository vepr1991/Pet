...
async def handle_booking_data(message: types.Message):
    ...
    try:
        ...
        # 1. Записываем в базу (включая client_name)
        db.add_appointment(
            user_id=message.from_user.id,
            breed=data.get('breed', 'Не указано'),
            pet_name=data.get('pet_name', 'Без клички'),
            service=data.get('service', 'Груминг'),
            date_time=dt,
            phone=data.get('phone'),
            master_id=m_id,
            client_name=client_tg_name  # Передаем имя сюда
        )
        ...
        notification = (
            ...
            f"🐶 <b>Питомец:</b> {data.get('breed')} ({data.get('pet_name')})\n"
            f"📅 <b>Время:</b> {dt}\n"
